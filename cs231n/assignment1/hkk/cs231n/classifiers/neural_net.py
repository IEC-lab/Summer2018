...
class TwoLayerNet(object):
  ...
  def loss(self, X, y=None, reg=0.0):
    ...
    exp_scores=np.exp(scores)/np.reshape(np.sum(np.exp(scores),axis=1),(N,-1))
    loss=-np.sum(np.log(exp_scores[range(N),y]))/N
    ...
    dh2=np.sum(dscores,axis=0)
    da2=np.dot(dscores,W2.T)
    da2[h1<=0]=0
    dW1=np.dot(X.T,da2)
    dh1=np.sum(da2,axis=0)
    dW2+=reg*W2
    dW1+=reg*W1
    grads['W1']=dW1
    grads['b1']=dh1
