...
```
    ...
    <ipython-input-5-84ea9f448f42> in <module>()
         15 net.initialize(ctx=ctx)
         16 w_key = []
    ---> 17 train(net, train_data, valid_data, num_epochs, learning_rate, lr_period, lr_decay, weight_decay, ctx, w_key, log_file, False, loss_f)
         18 net.save_params("models/shelock_densenet_orign")


    <ipython-input-4-45adbe072699> in train(net, train_data, valid_data, num_epochs, lr, lr_period, lr_decay, wd, ctx, w_key, output_file, verbose, loss_f)
         45             trainer.step(data.shape[0])
         46 
    ---> 47             _loss = nd.mean(loss).asscalar()
         48             _acc = utils.accuracy(output, label)
         49             #_acc = utils.accuracy(output, label)


    ~/.conda/envs/gluon/lib/python3.6/site-packages/mxnet/ndarray/ndarray.py in asscalar(self)
    ...
    ~/.conda/envs/gluon/lib/python3.6/site-packages/mxnet/ndarray/ndarray.py in asnumpy(self)
       1874             self.handle,
    ...
    ~/.conda/envs/gluon/lib/python3.6/site-packages/mxnet/base.py in check_call(ret)
        147     """
        148     if ret != 0:
    ...
    MXNetError: [14:29:45] src/storage/./pooled_storage_manager.h:108: cudaMalloc failed: out of memory
